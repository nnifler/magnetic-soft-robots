...
class TestAngles(unittest.TestCase):
    ...
    def testRandomized(self):
        ...
        for i, subscript in enumerate('xyz'):
            ...
            # sign error still persists, swapping order of compared vecs only changes axis in which they occur (y, if swapped x)
            a = MagneticController.calculate_angle(
                rand_vec, rotated_vec, subscript)
...
class TestRotation(unittest.TestCase):
    def testRandomized(self):
        ...
        for pair in zip(reference_vector, result_under_test):
            self.assertAlmostEqual(
                *pair, msg=f"{reference_vector} expected but found {result_under_test}, reference_rotation: {reference_rotation.as_euler('xyz')}, rot_under_test: {rot_under_test}")
    ...
    def testNoRot(self):
        ...
        reference_rotation = Rotation.from_euler(
            "xyz", noRot_angles, degrees=False)
        reference_vector = reference_rotation.apply(rand_base_vector)
    ...
    def testAntiParallel(self):
        ...
        for pair in zip(reverse_base_vector, result_under_test):
            self.assertAlmostEqual(
                *pair, msg=f"{reverse_base_vector} expected but found {result_under_test}", places=5)
