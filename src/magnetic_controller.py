...
class MagneticController(Sofa.Core.Controller):
    ...
    def calculate_angle(v1: np.ndarray, v2: np.ndarray, axis: str) -> float:
        ...
        p1, p2 = MagneticController._projection_from(
            axis, v1), MagneticController._projection_from(axis, v2)
    ...
    def calculate_rotation(normal: np.ndarray, initial_dipole_orientation: np.ndarray):
        ...
        if np.isclose(normal, initial_dipole_orientation).all():
            return Rotation.from_euler('x', 0)
        if np.isclose(normal, -1 * initial_dipole_orientation).all():
            normal = Rotation.from_euler(
                'xyz', [0.000000001*np.pi]*3).apply(normal)

        a = (normal / np.linalg.norm(normal)).reshape(3)
        b = (initial_dipole_orientation /
             np.linalg.norm(initial_dipole_orientation)).reshape(3)
        v = np.cross(a, b)
        c = np.dot(a, b)
        s = np.linalg.norm(v)
